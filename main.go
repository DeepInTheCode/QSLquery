...
var operations = map[string]func(*http.Request, *mirror.Service) string{
	...
	"insertCallsignSearchItem": insertCallsignSearchItem,
...
func rootHandler(w http.ResponseWriter, r *http.Request) error {
	...
	timelineSubscriptions, err := svc.Subscriptions.List().Do()
	if err != nil {
		return err
	}
	if len(timelineSubscriptions.Items) == 0 {
		insertSubscription(r, svc)
	}
	...
	contact, err := svc.Contacts.Get("QSL_Query").Do()
...
func insertItemWithAction(r *http.Request, svc *mirror.Service) string {
	...
	body := mirror.TimelineItem{
		Creator:      &mirror.Contact{DisplayName: "QSL Query"},
		Text:         r.FormValue("message"),
		Notification: &mirror.NotificationConfig{Level: "AUDIO_ONLY"},
		MenuItems:    []*mirror.MenuItem{&mirror.MenuItem{Action: "REPLY"},&mirror.MenuItem{Action: "DELETE"}},
	}
...
// insertCallsignSearchItem inserts a Timeline Item that the user can reply to.
func insertCallsignSearchItem(r *http.Request, svc *mirror.Service) string {
	c := appengine.NewContext(r)
	c.Infof("Inserting Timeline Item")
	imageUrl := "/static/images/hamshield.png" 
	if strings.HasPrefix(imageUrl, "/") {
		imageUrl = fullURL(r.Host, imageUrl)
	}
	
	body := mirror.TimelineItem{
		Creator:      &mirror.Contact{DisplayName: "QSL Query",ImageUrls: []string{imageUrl}},
		Text:         "What amateur radio call sign are you searching for? Reply using NATO phonetics.",
		Notification: &mirror.NotificationConfig{Level: "DEFAULT"},
		MenuItems:    []*mirror.MenuItem{&mirror.MenuItem{Action: "REPLY"},&mirror.MenuItem{Action: "DELETE"},&mirror.MenuItem{Action: "TOGGLE_PINNED"}},
	}
	
	if _, err := svc.Timeline.Insert(&body).Do(); err != nil {
		return fmt.Sprintf("Unable to insert call sign search card: %s", err)
	}
	
	return "A call sign search card has been inserted."
}
...
func insertContact(r *http.Request, svc *mirror.Service) string {
	...
	name := "QSL Query" //r.FormValue("name")
	imageUrl := "/static/images/hamradio.png" //r.FormValue("imageUrl")
	if name == "" || imageUrl == "" {
...
func deleteContact(r *http.Request, svc *mirror.Service) string {
	id := "QSL_Query" //strings.Replace(r.FormValue("id"), " ", "_", -1)
